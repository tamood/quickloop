...
class ChipyardAnalyzeEnv(gym.Env):

    def analyze_chipyard(self, action):
        buildDir = self.config.targetDir + ('/current' if action == (1 << 31 - 1) else f'/{action}')
        ...
        init_time = time()
        build_process = run(f'./update-targets.sh --sdk-path {self.config.edkDir} --target-name fast-qemu --target-type qemu --target-dts {buildDir}/{self.config.prefix}.dts',
                            shell=True, capture_output=True, text=True, cwd=f'{self.config.edkDir}/bsp', env=buid_env)
        print('Finished BSP build...')
        build_process = run('make PROGRAM=gemmini TARGET=fast-qemu LINK_TARGET=scratchpad clean',
                            shell=True, capture_output=True, text=True, cwd=self.config.edkDir, env=buid_env)
        print('Cleaned gemmini build...')
        build_process = run('make PROGRAM=gemmini TARGET=fast-qemu LINK_TARGET=scratchpad software',
                            shell=True, capture_output=True, text=True, cwd=self.config.edkDir, env=buid_env)
        ...
        set_reg {a0 0 a1 0x80000000 pc 0x80000000}
        resume
        exit"""
        ...
        with serial.Serial(f'{self.config.serial}', 115200, timeout=600) as ser:
            while True:
                ...
                try:
                    ...
                    if 'Workload Execution completed' in resultline:
                        break
                except:
                    break
            ser.close()
    ...
    def __init__(self, config):
        super(ChipyardAnalyzeEnv, self).__init__()
