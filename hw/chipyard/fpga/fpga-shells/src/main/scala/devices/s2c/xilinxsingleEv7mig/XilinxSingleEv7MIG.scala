...
import sifive.fpgashells.ip.xilinx.singleEv7mig.{SingleEv7MIGIOClocksReset, SingleEv7MIGIODDR, singleEv7mig}

case class XilinxSingleEv7MIGParams(
  address : Seq[AddressSet]
)

class XilinxSingleEv7MIGPads(depth : BigInt) extends SingleEv7MIGIODDR(depth) {
  def this(c : XilinxSingleEv7MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxSingleEv7MIGIO(depth : BigInt) extends SingleEv7MIGIODDR(depth) with SingleEv7MIGIOClocksReset

class XilinxSingleEv7MIGIsland(c : XilinxSingleEv7MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth<=0x200000000L),"singleEv7mig supports upto 8GB depth configuraton")
  ...
  lazy val module = new LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxSingleEv7MIGIO(depth)
    })
    ...
    val blackbox = Module(new singleEv7mig(depth))
...
class XilinxSingleEv7MIG(c : XilinxSingleEv7MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxSingleEv7MIGIsland(c, crossing))
